...
export async function POST(request: NextRequest) {
  try {
    ...
    const dbResult = await saveToMongoDB({
      ...
      originalImageUrl,
      cloudinaryPublicId: `lightx-${Date.now()}`, // We don't have the actual public ID for LightX results
      processedAt: new Date(),
      isLightXProcessed: true,
    })
    ...
    console.error("Error saving LightX result:", error)
